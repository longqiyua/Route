...
pub async fn skills_handler() -> Result<Json<Value>, ApiError> {
    route_cli::commands::extensions_skills().map_err(|e| ApiError::internal(e.to_string()))?;
    Ok(Json(json!({ "ok": true })))
}

/// `GET /api/extensions/references`
pub async fn references_handler() -> Result<Json<Value>, ApiError> {
    route_cli::commands::extensions_references().map_err(|e| ApiError::internal(e.to_string()))?;
    Ok(Json(json!({ "ok": true })))
}
